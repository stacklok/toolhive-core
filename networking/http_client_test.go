...
func TestNewHttpClientBuilder(t *testing.T) {
	...
	assert.NotNil(t, builder.envReader)
}
...
func TestNewHostScopedClientBuilder_InsecureDisableURLValidationEnvVar(t *testing.T) {
	...
	builder := NewHostScopedClientBuilder(testIdpExampleHost, false, false)
...
func TestHttpClientBuilder_WithEnvReader(t *testing.T) {
	t.Parallel()
	...
	builder := NewHttpClientBuilder()
	result := builder.WithEnvReader(mockReader)

	assert.Same(t, builder, result) // fluent interface
	assert.Same(t, mockReader, builder.envReader)
}
...
-----END CERTIFICATE-----`
		...
		},
		{
			...
			validateClient: func(t *testing.T, client *http.Client) {
				...
				transport := client.Transport.(*ValidatingTransport)
				httpTransport := transport.Transport.(*http.Transport)
				assert.Nil(t, httpTransport.DialContext, "dial guard must be absent when private IPs are allowed")
				assert.False(t, httpTransport.DisableKeepAlives, "keep-alives may stay enabled when no dial guard is installed")
			},
		},
		{
			...
			validateClient: func(t *testing.T, client *http.Client) {
				...
				transport := client.Transport.(*ValidatingTransport)
				httpTransport := transport.Transport.(*http.Transport)
				assert.NotNil(t, httpTransport.DialContext, "dial guard must be installed when private IPs are disallowed")
				assert.True(t, httpTransport.DisableKeepAlives,
					"keep-alives must be disabled whenever the dial guard is installed, "+
						"otherwise a pooled connection skips the per-dial check on later requests")
			},
		},
		{
		...
		},
		{
			...
			expectError:   true,
			errorContains: testAuthTokenEmptyErrMsg,
		},
		{
...
// TestHttpClientBuilder_Build_KeepAlivePairing pins that the per-dial SSRF
// guard and DisableKeepAlives can only appear together: a pooled, kept-alive
// connection would otherwise reuse a previously-validated dial and skip the
// guard on later requests through that connection.
func TestHttpClientBuilder_Build_KeepAlivePairing(t *testing.T) {
	t.Parallel()

	t.Run("private IPs disallowed: guard installed, keep-alives disabled", func(t *testing.T) {
		t.Parallel()

		client, err := NewHttpClientBuilder().WithPrivateIPs(false).Build()
	...
	t.Run("private IPs allowed: guard absent, keep-alives left enabled", func(t *testing.T) {
		t.Parallel()
...
// TestValidatingTransport_RoundTrip_EnvReader pins that RoundTrip consults
// the injected env.Reader (rather than the process environment) for
// INSECURE_DISABLE_URL_VALIDATION, and that a nil EnvReader falls back to the
// real OS environment.
func TestValidatingTransport_RoundTrip_EnvReader(t *testing.T) {
	t.Parallel()

	t.Run("injected reader reporting true skips validation", func(t *testing.T) {
		t.Parallel()
		...
		mockTransport := &mockRoundTripper{}
		transport := &ValidatingTransport{Transport: mockTransport, EnvReader: mockReader}
	...
	t.Run("injected reader reporting false still enforces HTTPS", func(t *testing.T) {
		t.Parallel()

		ctrl := gomock.NewController(t)
		mockReader := mocks.NewMockReader(ctrl)
		mockReader.EXPECT().Getenv("INSECURE_DISABLE_URL_VALIDATION").Return("false")
		...
		assert.False(t, mockTransport.called)
	})

}

// TestValidatingTransport_RoundTrip_NilEnvReaderUsesOS pins that a nil
// EnvReader falls back to the real OS environment. Kept as its own
// non-parallel top-level test because t.Setenv panics if any ancestor test
// has called t.Parallel.
func TestValidatingTransport_RoundTrip_NilEnvReaderUsesOS(t *testing.T) {
	t.Setenv("INSECURE_DISABLE_URL_VALIDATION", "true")

	mockTransport := &mockRoundTripper{}
	transport := &ValidatingTransport{Transport: mockTransport}
...
func TestSameHostRedirectPolicy_CapsChain(t *testing.T) {
	...
	for i := range via {
		via[i] = mustReq(t, testMCPStartURL)
	}
