...
// straight from caller input (typically a CLI flag), so this is the
// recognized sanitization barrier before the path is opened — no
// string-inspection shortcuts.
...
func loadKeypair(path string) (sign.Keypair, error) {
	...
	pemBytes, err := os.ReadFile(resolved) //nolint:gosec // path is canonicalized and vetted by resolveKeyPath
	if err != nil {
